 ...
 *
 * <p>The work has three steps, and each one has its own operation: read the XML, make the files
 * of the posts (which resolves each post's true size internally), save them. The two first steps
 * make no write in the database, and the last one makes no connection to the news server.</p>
 */
@Service
public class NzbProcessingService {
     ...
     * only writes.</p>
     *
     * @throws NzbParseException if the XML of the NZB is not readable.
     */
